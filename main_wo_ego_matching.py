...
def main(args_in=None):
    ...
    # Initialize default values for transformations
    ...
    # When ONLY_EXO is True, transform exo pointcloud to ego space and render in ego view
    if ONLY_EXO:
        # Transform exo pointcloud to ego space
        transformed_exo_total_pcd = copy.deepcopy(exo_total_pcd)
        transformed_exo_total_pcd.transform(exo_to_ego_transform)

        o3d.io.write_point_cloud(
            f"{args.out_folder}/{os.path.basename(args.ego_image).replace('.png', '')}_exo_in_ego_space.ply",
            transformed_exo_total_pcd,
        )
        print(
            f"Saved transformed exo pointcloud in ego space to {args.out_folder}/{os.path.basename(args.ego_image).replace('.png', '')}_exo_in_ego_space.ply"
        )

        # Extract exo pointcloud RGB and positions for rendering
        exo_points = np.asarray(transformed_exo_total_pcd.points)
        exo_colors = np.asarray(transformed_exo_total_pcd.colors)

        # Get ego camera parameters for rendering
        ego_scaled_focal_length = ego_params["focal_length"]
        ego_img_size = ego_params["img_size"].cpu().numpy()

        # Create a renderer to render the exo points from ego camera view
        try:
            from utils.visualization import render_points_in_camera_view

            # Render transformed exo points from ego camera view
            rendered_exo_in_ego = render_points_in_camera_view(
                points=exo_points,
                colors=exo_colors,
                focal_length=ego_scaled_focal_length,
                img_size=ego_img_size,
                cam_ext=ego_cam_ext,
            )

            # Save the rendered image
            cv2.imwrite(
                f"{args.out_folder}/{os.path.basename(args.ego_image).replace('.png', '')}_exo_in_ego_view.png",
                rendered_exo_in_ego,
            )
            print(
                f"Saved rendered exo points in ego view to {args.out_folder}/{os.path.basename(args.ego_image).replace('.png', '')}_exo_in_ego_view.png"
            )
        except ImportError:
            print("Couldn't import render_points_in_camera_view - skipping rendering")
            # Alternative: use exo2ego_force as a fallback if available
            try:
                # Extract camera paths from the original image paths
                exo_rgb_path = args.exo_image
                exo_cam_id = exo_rgb_path.split("/")[
                    -3
                ]  # Assuming path format .../camX/rgb/image.png
                ego_cam_id = args.ego_image.split("/")[-3]

                # Prepare paths for exo2ego_force function
                exo_rgb_id = os.path.join("rgb", exo_rgb_path.split("/")[-1])
                exo_depth_path = exo_rgb_path.replace("rgb", "depth")
                exo_cam_int_path = exo_rgb_path.replace(
                    exo_rgb_id, "cam_intrinsics.txt"
                )
                exo_cam_ext_path = exo_rgb_path.replace("rgb", "cam_pose").replace(
                    "png", "txt"
                )
                exo_hand_path = exo_rgb_path.replace("rgb", "hand_pose").replace(
                    "png", "txt"
                )
                ...
                # Generate exo view in ego camera space
                ...
                # Save the rendered image
                cv2.imwrite(
                    f"{args.out_folder}/{os.path.basename(args.exo_image).replace('.png', '')}_in_ego_view.png",
                    ego_rgb_pred,
                )
                print(
                    f"Saved exo2ego transformed view to {args.out_folder}/{os.path.basename(args.exo_image).replace('.png', '')}_in_ego_view.png"
                )
            except Exception as e:
                print(f"Error using exo2ego_force: {e}")
    else:
        # Original code for non-ONLY_EXO mode
